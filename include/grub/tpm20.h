...
#ifndef GRUB_TPM20_HEADER
#define GRUB_TPM20_HEADER 1

// Vendor specifics? These are taken from the Intel tpm2-tss library
#define  MAX_RSA_KEY_BYTES          256
#define  MAX_ECC_KEY_BYTES     48
#define CRT_FORMAT_RSA      1 /* YES */
...
#define MAX_RSA_KEY_grub_uint8_tS ((MAX_RSA_KEY_BITS + 7) / 8)

// Table 216 - Defines for ECC Algorithm Constants
#define MAX_ECC_KEY_BITS  256
#define MAX_ECC_KEY_grub_uint8_tS ((MAX_ECC_KEY_BITS + 7) / 8)

// Table 217 - Defines for AES Algorithm Constants
#define MAX_AES_KEY_BITS         128
#define MAX_AES_BLOCK_SIZE_grub_uint8_tS 16
#define MAX_AES_KEY_grub_uint8_tS        ((MAX_AES_KEY_BITS + 7) / 8)

// Table 218 - Defines for SM4 Algorithm Constants
#define MAX_SM4_KEY_BITS         128
#define MAX_SM4_BLOCK_SIZE_grub_uint8_tS 16
#define MAX_SM4_KEY_grub_uint8_tS        ((MAX_SM4_KEY_BITS + 7) / 8)

// Table 219 - Defines for Symmetric Algorithm Constants
#define MAX_SYM_KEY_BITS   MAX_AES_KEY_BITS
#define MAX_SYM_KEY_grub_uint8_tS  MAX_AES_KEY_BYTES
#define MAX_SYM_BLOCK_SIZE MAX_AES_BLOCK_SIZE_grub_uint8_tS

// Table 220 - Defines for Implementation Values
typedef grub_uint16_t                        BSIZE;
...
typedef grub_uint16_t                        CONTEXT_SLOT;
typedef grub_uint64_t                        CONTEXT_COUNTER;
...
#define CONTEXT_ENCRYPT_KEY_grub_uint8_tS     ((CONTEXT_ENCRYPT_KEY_BITS + 7) / 8)
...
#define PRIVATE_VENDOR_SPECIFIC_grub_uint8_tS ((MAX_RSA_KEY_BYTES / 2) * ( 3 + CRT_FORMAT_RSA * 2))

// Capability related MAX_ value
#define MAX_CAP_DATA       (MAX_CAP_BUFFER - sizeof(TPM_CAP) - sizeof(grub_uint32_t))
...
typedef grub_uint32_t TPM_ALGORITHM_ID;
typedef grub_uint32_t TPM_MODIFIER_INDICATOR;
typedef grub_uint32_t TPM_AUTHORIZATION_SIZE;
typedef grub_uint32_t TPM_PARAMETER_SIZE;
typedef grub_uint16_t TPM_KEY_SIZE;
...
typedef grub_int8_t TPM_CLOCK_ADJUST;
...
typedef grub_uint32_t TPM_PT_PCR;
...
typedef struct {
  grub_uint32_t asymmetric    : 1;
  grub_uint32_t symmetric     : 1;
  grub_uint32_t hash          : 1;
  grub_uint32_t object        : 1;
  grub_uint32_t reserved4_7   : 4;
  grub_uint32_t signing       : 1;
  grub_uint32_t encrypting    : 1;
  grub_uint32_t method        : 1;
  grub_uint32_t reserved11_31 : 21;
} GRUB_PACKED GRUB_PACKED TPMA_ALGORITHM;
...
//
typedef struct {
  grub_uint8_t locZero  : 1;
  grub_uint8_t locOne   : 1;
  grub_uint8_t locTwo   : 1;
  grub_uint8_t locThree : 1;
  grub_uint8_t locFour  : 1;
  grub_uint8_t Extended : 3;
} GRUB_PACKED TPMA_LOCALITY;

// Table 33 - TPMA_PERMANENT Bits
typedef struct {
  grub_uint32_t ownerAuthSet       : 1;
  grub_uint32_t endorsementAuthSet : 1;
  grub_uint32_t lockoutAuthSet     : 1;
  grub_uint32_t reserved3_7        : 5;
  grub_uint32_t disableClear       : 1;
  grub_uint32_t inLockout          : 1;
  grub_uint32_t tpmGeneratedEPS    : 1;
  grub_uint32_t reserved11_31      : 21;
} GRUB_PACKED TPMA_PERMANENT;

// Table 34 - TPMA_STARTUP_CLEAR Bits
typedef struct {
  grub_uint32_t phEnable     : 1;
  grub_uint32_t shEnable     : 1;
  grub_uint32_t ehEnable     : 1;
  grub_uint32_t reserved3_30 : 28;
  grub_uint32_t orderly      : 1;
} GRUB_PACKED TPMA_STARTUP_CLEAR;

// Table 35 - TPMA_MEMORY Bits
typedef struct {
  grub_uint32_t sharedRAM         : 1;
  grub_uint32_t sharedNV          : 1;
  grub_uint32_t objectCopiedToRam : 1;
  grub_uint32_t reserved3_31      : 29;
} GRUB_PACKED TPMA_MEMORY;

// Table 36 - TPMA_CC Bits
typedef struct {
  grub_uint32_t commandIndex  : 16;
  grub_uint32_t reserved16_21 : 6;
  grub_uint32_t nv            : 1;
  grub_uint32_t extensive     : 1;
  grub_uint32_t flushed       : 1;
  grub_uint32_t cHandles      : 3;
  grub_uint32_t rHandle       : 1;
  grub_uint32_t V             : 1;
  grub_uint32_t Res           : 2;
} GRUB_PACKED TPMA_CC;
...
} GRUB_PACKED TPMS_ALGORITHM_DESCRIPTION;

// Table 66 - TPMU_HA Union
typedef union {
  grub_uint8_t sha1[SHA1_DIGEST_SIZE];
  grub_uint8_t sha256[SHA256_DIGEST_SIZE];
  grub_uint8_t sm3_256[SM3_256_DIGEST_SIZE];
  grub_uint8_t sha384[SHA384_DIGEST_SIZE];
  grub_uint8_t sha512[SHA512_DIGEST_SIZE];
} GRUB_PACKED TPMU_HA;
...
} GRUB_PACKED TPM2B_EVENT;

// Table 74 - TPM2B_MAX_BUFFER Structure
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[MAX_DIGEST_BUFFER];
} GRUB_PACKED TPM2B_MAX_BUFFER;

// Table 75 - TPM2B_MAX_NV_BUFFER Structure
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[MAX_NV_INDEX_SIZE];
} GRUB_PACKED TPM2B_MAX_NV_BUFFER;

// Table 76 - TPM2B_TIMEOUT Structure
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[sizeof(grub_uint64_t)];
} GRUB_PACKED TPM2B_TIMEOUT;

// Table 77 -- TPM2B_IV Structure <I/O>
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[MAX_SYM_BLOCK_SIZE];
} GRUB_PACKED TPM2B_IV;

// Table 78 - TPMU_NAME Union
typedef union {
  TPMT_HA    digest;
  TPM_HANDLE handle;
} GRUB_PACKED TPMU_NAME;

// Table 79 - TPM2B_NAME Structure
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   name[sizeof(TPMU_NAME)];
} GRUB_PACKED TPM2B_NAME;

// Table 80 - TPMS_PCR_SELECT Structure
typedef struct {
  grub_uint8_t sizeofSelect;
  grub_uint8_t  pcrSelect[PCR_SELECT_MAX];
} GRUB_PACKED TPMS_PCR_SELECT;
...
typedef struct {
  ...
  grub_uint8_t         sizeofSelect;
  grub_uint8_t          pcrSelect[PCR_SELECT_MAX];
} GRUB_PACKED TPMS_PCR_SELECTION;
...
typedef struct {
  ...
  TPMI_RH_HIERARCHY hierarchy;
  TPM2B_DIGEST      digest;
} GRUB_PACKED TPMT_TK_HASHCHECK;

// Table 88 - TPMS_ALG_PROPERTY Structure
typedef struct {
  TPM_ALG_ID     alg;
  TPMA_ALGORITHM algProperties;
} GRUB_PACKED TPMS_ALG_PROPERTY;

// Table 89 - TPMS_TAGGED_PROPERTY Structure
typedef struct {
  TPM_PT property;
  grub_uint32_t value;
} GRUB_PACKED TPMS_TAGGED_PROPERTY;
...
typedef struct {
  ...
  grub_uint8_t  sizeofSelect;
  grub_uint8_t   pcrSelect[PCR_SELECT_MAX];
} GRUB_PACKED TPMS_TAGGED_PCR_SELECT;
...
} GRUB_PACKED TPMU_CAPABILITIES;

// Table 104 - TPMS_CAPABILITY_DATA Structure
typedef struct {
  TPM_CAP           capability;
  TPMU_CAPABILITIES data;
} GRUB_PACKED TPMS_CAPABILITY_DATA;

// Table 105 - TPMS_CLOCK_INFO Structure
typedef struct {
  grub_uint64_t      clock;
  grub_uint32_t      resetCount;
  grub_uint32_t      restartCount;
  TPMI_YES_NO safe;
} GRUB_PACKED TPMS_CLOCK_INFO;

// Table 106 - TPMS_TIME_INFO Structure
typedef struct {
  grub_uint64_t          time;
  TPMS_CLOCK_INFO clockInfo;
} GRUB_PACKED TPMS_TIME_INFO;

// Table 107 - TPMS_TIME_ATTEST_INFO Structure
typedef struct {
  TPMS_TIME_INFO time;
  grub_uint64_t         firmwareVersion;
} GRUB_PACKED TPMS_TIME_ATTEST_INFO;

// Table 108 - TPMS_CERTIFY_INFO Structure
typedef struct {
  TPM2B_NAME name;
  TPM2B_NAME qualifiedName;
} GRUB_PACKED TPMS_CERTIFY_INFO;

// Table 109 - TPMS_QUOTE_INFO Structure
typedef struct {
  TPML_PCR_SELECTION pcrSelect;
  TPM2B_DIGEST       pcrDigest;
} GRUB_PACKED TPMS_QUOTE_INFO;

// Table 110 - TPMS_COMMAND_AUDIT_INFO Structure
typedef struct {
  grub_uint64_t       auditCounter;
  TPM_ALG_ID   digestAlg;
  TPM2B_DIGEST auditDigest;
  TPM2B_DIGEST commandDigest;
} GRUB_PACKED TPMS_COMMAND_AUDIT_INFO;

// Table 111 - TPMS_SESSION_AUDIT_INFO Structure
typedef struct {
  TPMI_YES_NO  exclusiveSession;
  TPM2B_DIGEST sessionDigest;
} GRUB_PACKED TPMS_SESSION_AUDIT_INFO;

// Table 112 - TPMS_CREATION_INFO Structure
typedef struct {
  TPM2B_NAME   objectName;
  TPM2B_DIGEST creationHash;
} GRUB_PACKED TPMS_CREATION_INFO;
...
} GRUB_PACKED TPMS_NV_CERTIFY_INFO;
...
} GRUB_PACKED TPMS_ATTEST;

// Table 117 - TPM2B_ATTEST Structure
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   attestationData[sizeof(TPMS_ATTEST)];
} GRUB_PACKED TPM2B_ATTEST;
...
typedef struct {
  ...
  TPMA_SESSION sessionAttributes;
  TPM2B_AUTH   hmac;
} GRUB_PACKED TPMS_AUTH_RESPONSE;
...
typedef struct {
  ...
  TPMU_SYM_KEY_BITS   keyBits;
  TPMU_SYM_MODE       mode;
} GRUB_PACKED TPMT_SYM_DEF_OBJECT;

// Table 127 - TPM2B_SYM_KEY Structure
/* Note: this is almost certainly not correct! */
#define MAX_SYM_KEY_BYTES 32
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[MAX_SYM_KEY_BYTES];
} GRUB_PACKED TPM2B_SYM_KEY;

// Table 128 - TPMS_SYMCIPHER_PARMS Structure
typedef struct {
  TPMT_SYM_DEF_OBJECT sym;
} GRUB_PACKED TPMS_SYMCIPHER_PARMS;

// Table 129 - TPM2B_SENSITIVE_DATA Structure
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[MAX_SYM_DATA];
} GRUB_PACKED TPM2B_SENSITIVE_DATA;

// Table 130 - TPMS_SENSITIVE_CREATE Structure
typedef struct {
  TPM2B_AUTH           userAuth;
  TPM2B_SENSITIVE_DATA data;
} GRUB_PACKED TPMS_SENSITIVE_CREATE;

// Table 131 - TPM2B_SENSITIVE_CREATE Structure
typedef struct {
  grub_uint16_t                size;
  TPMS_SENSITIVE_CREATE sensitive;
} GRUB_PACKED TPM2B_SENSITIVE_CREATE;

// Table 132 - TPMS_SCHEME_SIGHASH Structure
typedef struct {
  TPMI_ALG_HASH hashAlg;
} GRUB_PACKED TPMS_SCHEME_SIGHASH;
...
typedef struct {
  TPMI_ALG_HASH hashAlg;
  TPMI_ALG_KDF  kdf;
} GRUB_PACKED TPMS_SCHEME_XOR;

// Table 136 - TPMU_SCHEME_KEYEDHASH Union
typedef union {
  TPMS_SCHEME_HMAC hmac;
  TPMS_SCHEME_XOR  xor;
} GRUB_PACKED TPMU_SCHEME_KEYEDHASH;

// Table 137 - TPMT_KEYEDHASH_SCHEME Structure
typedef struct {
  TPMI_ALG_KEYEDHASH_SCHEME scheme;
  TPMU_SCHEME_KEYEDHASH     details;
} GRUB_PACKED TPMT_KEYEDHASH_SCHEME;
...
} GRUB_PACKED TPMU_KDF_SCHEME;

// Table 150 - TPMT_KDF_SCHEME Structure
typedef struct {
  TPMI_ALG_KDF    scheme;
  TPMU_KDF_SCHEME details;
} GRUB_PACKED TPMT_KDF_SCHEME;
...
} GRUB_PACKED TPMU_ASYM_SCHEME;

// Table 153 - TPMT_ASYM_SCHEME Structure
typedef struct {
  TPMI_ALG_ASYM_SCHEME scheme;
  TPMU_ASYM_SCHEME     details;
} GRUB_PACKED TPMT_ASYM_SCHEME;
...
typedef struct {
  ...
  TPMU_ASYM_SCHEME     details;
} GRUB_PACKED TPMT_RSA_DECRYPT;

// Table 158 - TPM2B_PUBLIC_KEY_RSA Structure
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[MAX_RSA_KEY_BYTES];
} GRUB_PACKED TPM2B_PUBLIC_KEY_RSA;
...
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[MAX_RSA_KEY_BYTES/2];
} GRUB_PACKED TPM2B_PRIVATE_KEY_RSA;

// Table 161 - TPM2B_ECC_PARAMETER Structure
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[MAX_ECC_KEY_BYTES];
} GRUB_PACKED TPM2B_ECC_PARAMETER;

// Table 162 - TPMS_ECC_POINT Structure
typedef struct {
  TPM2B_ECC_PARAMETER x;
  TPM2B_ECC_PARAMETER y;
} GRUB_PACKED TPMS_ECC_POINT;

// Table 163 -- TPM2B_ECC_POINT Structure <I/O>
typedef struct {
  grub_uint16_t         size;
  TPMS_ECC_POINT point;
} GRUB_PACKED TPM2B_ECC_POINT;
...
} GRUB_PACKED TPMS_ALGORITHM_DETAIL_ECC;

// Table 168 - TPMS_SIGNATURE_RSASSA Structure
typedef struct {
  TPMI_ALG_HASH        hash;
  TPM2B_PUBLIC_KEY_RSA sig;
} GRUB_PACKED TPMS_SIGNATURE_RSASSA;

// Table 169 - TPMS_SIGNATURE_RSAPSS Structure
typedef struct {
  TPMI_ALG_HASH        hash;
  TPM2B_PUBLIC_KEY_RSA sig;
} GRUB_PACKED TPMS_SIGNATURE_RSAPSS;
...
} GRUB_PACKED TPMS_SIGNATURE_ECDSA;
...
} GRUB_PACKED TPMU_PUBLIC_PARMS;

// Table 182 - TPMT_PUBLIC_PARMS Structure
typedef struct {
  TPMI_ALG_PUBLIC   type;
  TPMU_PUBLIC_PARMS parameters;
} GRUB_PACKED TPMT_PUBLIC_PARMS;
...
} GRUB_PACKED TPMT_PUBLIC;

// Table 184 - TPM2B_PUBLIC Structure
typedef struct {
  grub_uint16_t      size;
  TPMT_PUBLIC publicArea;
} GRUB_PACKED TPM2B_PUBLIC;

// Table 185 - TPM2B_PRIVATE_VENDOR_SPECIFIC Structure
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[PRIVATE_VENDOR_SPECIFIC_BYTES];
} GRUB_PACKED TPM2B_PRIVATE_VENDOR_SPECIFIC;
...
} GRUB_PACKED TPMU_SENSITIVE_COMPOSITE;
...
} GRUB_PACKED TPMT_SENSITIVE;

// Table 188 - TPM2B_SENSITIVE Structure
typedef struct {
  grub_uint16_t         size;
  TPMT_SENSITIVE sensitiveArea;
} GRUB_PACKED TPM2B_SENSITIVE;
...
//
//typedef struct {
//  grub_uint32_t index : 22;
//  grub_uint32_t space : 2;
//  grub_uint32_t RH_NV : 8;
//} GRUB_PACKED TPM_NV_INDEX;

// Table 195 - TPMA_NV Bits
typedef struct {
  grub_uint32_t TPMA_NV_PPWRITE        : 1;
  grub_uint32_t TPMA_NV_OWNERWRITE     : 1;
  grub_uint32_t TPMA_NV_AUTHWRITE      : 1;
  grub_uint32_t TPMA_NV_POLICYWRITE    : 1;
  grub_uint32_t TPMA_NV_COUNTER        : 1;
  grub_uint32_t TPMA_NV_BITS           : 1;
  grub_uint32_t TPMA_NV_EXTEND         : 1;
  grub_uint32_t reserved7_9            : 3;
  grub_uint32_t TPMA_NV_POLICY_DELETE  : 1;
  grub_uint32_t TPMA_NV_WRITELOCKED    : 1;
  grub_uint32_t TPMA_NV_WRITEALL       : 1;
  grub_uint32_t TPMA_NV_WRITEDEFINE    : 1;
  grub_uint32_t TPMA_NV_WRITE_STCLEAR  : 1;
  grub_uint32_t TPMA_NV_GLOBALLOCK     : 1;
  grub_uint32_t TPMA_NV_PPREAD         : 1;
  grub_uint32_t TPMA_NV_OWNERREAD      : 1;
  grub_uint32_t TPMA_NV_AUTHREAD       : 1;
  grub_uint32_t TPMA_NV_POLICYREAD     : 1;
  grub_uint32_t reserved20_24          : 5;
  grub_uint32_t TPMA_NV_NO_DA          : 1;
  grub_uint32_t TPMA_NV_ORDERLY        : 1;
  grub_uint32_t TPMA_NV_CLEAR_STCLEAR  : 1;
  grub_uint32_t TPMA_NV_READLOCKED     : 1;
  grub_uint32_t TPMA_NV_WRITTEN        : 1;
  grub_uint32_t TPMA_NV_PLATFORMCREATE : 1;
  grub_uint32_t TPMA_NV_READ_STCLEAR   : 1;
} GRUB_PACKED TPMA_NV;
...
} GRUB_PACKED TPMS_NV_PUBLIC;

// Table 197 - TPM2B_NV_PUBLIC Structure
typedef struct {
  grub_uint16_t         size;
  TPMS_NV_PUBLIC nvPublic;
} GRUB_PACKED TPM2B_NV_PUBLIC;
...
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[MAX_CONTEXT_SIZE];
} GRUB_PACKED TPM2B_CONTEXT_SENSITIVE;

// Table 199 - TPMS_CONTEXT_DATA Structure
typedef struct {
  TPM2B_DIGEST            integrity;
  TPM2B_CONTEXT_SENSITIVE encrypted;
} GRUB_PACKED TPMS_CONTEXT_DATA;

// Table 200 - TPM2B_CONTEXT_DATA Structure
typedef struct {
  grub_uint16_t size;
  grub_uint8_t   buffer[sizeof(TPMS_CONTEXT_DATA)];
} GRUB_PACKED TPM2B_CONTEXT_DATA;
...
} GRUB_PACKED TPMS_CREATION_DATA;

// Table 204 - TPM2B_CREATION_DATA Structure
typedef struct {
  grub_uint16_t             size;
  TPMS_CREATION_DATA creationData;
} GRUB_PACKED TPM2B_CREATION_DATA;
...
typedef struct {
  ...
  TPM_CC commandCode;
} GRUB_PACKED TPM2_COMMAND_HEADER;

typedef struct {
  TPM_ST tag;
  grub_uint32_t paramSize;
  TPM_RC responseCode;
} GRUB_PACKED TPM2_RESPONSE_HEADER;


#endif /* GRUB_TPM_HEADER */
